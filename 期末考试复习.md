...
public class Address{
    ...
    // 别忘了使用ctrl+shift+s生成getter和setter方法
}
...
AOP的重点在于切点和切面, 并且需要配置好bean和bean中的方法才能够使用aop进行方法的增强

### 使用xml方式配置Aop、切点、前置、后置通知

提前把前置通知后置通知的方法定义好，再通过spring配置文件把切点、切面配置上
...
使用注解把bean放入spring容器，再使用注解把aop配置类放入容器，在配置类中使用@Pointcut和@Before@After配置切面方法。
...
## 第12章JDBCTemplate与事务管理 
* JDBCTemplate是spring提供的持久化工具，跟mybatis一样可以访问数据库
使用方法：接口类见书本page212 
### 事务管理 
* 使用xml配置事务
事务加载Service层方法上，spring事务是基于springaop实现的，假设需要加事务的方法transfer在*spring.jdbc.service.impl.UserServiceImpl中*:
1. 在spring配置文件中加入事务管理器
```xml
...
```xml
<aop:config>
  <aop:pointcut expression="execution(* spring.jdbc.service.impl.UserServiceImpl.*(..))" id="myPointcut"></aop:pointcut>
  <aop:advisor advice-ref="myAdvice" pointcut-ref="myPointcut"></aop:advisor>
</aop:config>
```

* 使用注解方式配置事务
1. 在spring的配置文件中开启注解事务
2. 在需要事务的方法或类上使用@Transactional注解

## 第13章 SpringMVC基础

可能会考@RequestMapping、@RestController...等注解

配置试图解析器相关的我觉得不考，考了吃一斤
